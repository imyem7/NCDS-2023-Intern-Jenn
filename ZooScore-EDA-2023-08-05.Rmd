...
p_all<- GID %>%
  left_join(MDD[, c("species", "order")], by = "species") %>% #subsets the MDD dataset (species names and taxonomic information) to a couple of columns of interest and then joins them
  left_join(ZOO, by = c("ParasiteGMPD" = "parasite_corrected_name")) %>%
  group_by(ParasiteGMPD, order) %>% #we add order to our grouping here to make it easier to calculate species richness within each group
...
## 5-5. Speceis Richness across Country
